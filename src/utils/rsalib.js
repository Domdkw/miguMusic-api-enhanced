...
globalThis.define = function (id, deps, factory) {
  // 统一处理参数：define(字符串ID, 空数组, 函数)
  let req = () => ({});
  let exp = {};
  let mod = { exports: exp };

  // 执行工厂，严格传入 a=require, b=exports, c=module
  factory(req, exp, mod);
...
globalThis.window.crypto = {
  getRandomValues: function(arr) {
    // 使用 crypto-js 生成随机字节
    const bytes = CryptoJS.lib.WordArray.random(arr.length).toString(CryptoJS.enc.Latin1);
  ...
  random: function(len) {
    // 适配旧版 random 方法，返回字符数组
    return CryptoJS.lib.WordArray.random(len).toString(CryptoJS.enc.Latin1);
  }
};
...
define("lib/rsa/rsa", [], function(a, b, c) {
    ...
    function eb(a, b) {
        ...
        this.e = parseInt(b, 16)) : console.error('net error')
    }
    function fb(a) {
...
//inject
export const RSAKey = rsa?.RSAKey;
export default rsa;